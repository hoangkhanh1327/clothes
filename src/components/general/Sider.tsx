...
import { deleteAccountTypeAsync, toggleTypeFormVisible } from '../../redux/reducers/accountSlice'
...
const HomeSider: React.FC<{}> = ({}) => {
  ...
  const handleClickAccountTypeDropdown = (props: any, accountType: AccountTypeType) => {
    let key = props.key || ''
    switch (key) {
      ...
      case 'deleteAccountType':
        confirm({
          ...
          cancelText: 'Huỷ',
          onOk() {
            dispatch(deleteAccountTypeAsync(accountType.id as number))
          },
          onCancel() {}
        })
        return
  ...
  useEffect(() => {
    if (accountTypes.length) {
        ...
        return getItem(
          ...
          <div className='tw-relative tw-group group-[] '>
            ...
            <Dropdown
              menu={{
                items,
                onClick: (props) => handleClickAccountTypeDropdown(props, accountType)
              }}
              trigger={['click']}
            ...
            >
              ...
              >
                <Space>
                  <Tooltip title='Chính sửa' placement='right'>
      ...
      accountMenu.unshift(getItem(`account-type-add-new`, 'Tất cả', '/tai-khoan/'))
      accountMenu.push(getItem(`account-type-add-new`, 'Thêm Loại Tài Khoản', 'addNewAccountType'))
